...
<body class="antialiased text-slate-800 bg-[#f0f9f4] dark:bg-emerald-950 dark:text-emerald-50 transition-colors duration-300">
    ...
    <div class="flex min-h-screen relative overflow-hidden">
            ...
            <nav class="flex-1 px-4 mt-6 overflow-y-auto custom-scrollbar space-y-1">
                ...
                @if(auth()->user()->role === 'admin')
                    ...
                    <a href="{{ route('admin.master.user.index') }}" class="nav-item flex items-center py-4 px-5 rounded-2xl transition-all {{ Request::is('admin/master/user*') ? 'sidebar-active' : 'hover:bg-white/10' }}">
                        ...
                        <span class="nav-text ml-3 text-sm font-bold tracking-wide text-nowrap">Kelola User</span>
                    </a>
                    <a href="{{ route('admin.master.kategori.index') }}" class="nav-item flex items-center py-4 px-5 rounded-2xl transition-all {{ Request::is('admin/master/kategori*') ? 'sidebar-active' : 'hover:bg-white/10' }}">
                        ...
                        <span class="nav-text ml-3 text-sm font-bold tracking-wide text-nowrap">Kategori Surat</span>
                    </a>
                    <a href="{{ route('admin.master.instruksi.index') }}" class="nav-item flex items-center py-4 px-5 rounded-2xl transition-all {{ Request::is('admin/master/instruksi*') ? 'sidebar-active' : 'hover:bg-white/10' }}">
                        ...
                        <span class="nav-text ml-3 text-sm font-bold tracking-wide text-nowrap">Master Instruksi</span>
                    </a>
                    ...
                    <a href="{{ route('admin.aktivitas.index') }}" class="nav-item flex items-center py-4 px-5 rounded-2xl transition-all {{ Request::is('admin/aktivitas*') ? 'sidebar-active' : 'hover:bg-white/10' }}">
                        ...
                        <span class="nav-text ml-3 text-sm font-bold tracking-wide text-nowrap">Audit Log</span>
                    </a>
                @endif
                ...
                @if(auth()->user()->role === 'petugas')
                    ...
                    <a href="{{ route('petugas.surat.index') }}" class="nav-item flex items-center py-4 px-5 rounded-2xl transition-all {{ Request::is('petugas/manajemen_surat*') ? 'sidebar-active' : 'hover:bg-white/10' }}">
                        <i class="fas fa-envelope-open-text w-6 text-center text-sm"></i>
                        <span class="nav-text ml-3 text-sm font-bold tracking-wide text-nowrap">Digitalisasi Surat</span>
                    </a>
                    <a href="{{ route('petugas.arsip.index') }}" class="nav-item flex items-center py-4 px-5 rounded-2xl transition-all {{ Request::is('petugas/manajemen_arsip*') ? 'sidebar-active' : 'hover:bg-white/10' }}">
                ...
                <!-- ==========================================
                     AKTOR: PIMPINAN (Approval & Monitoring)
                =========================================== -->
                @if(auth()->user()->role === 'pimpinan')
                    ...
                    <a href="{{ route('pimpinan.instruksi-surat.index') }}" class="nav-item flex items-center py-4 px-5 rounded-2xl transition-all {{ Request::is('pimpinan/instruksi_surat*') ? 'sidebar-active' : 'hover:bg-white/10' }}">
                        <i class="fas fa-file-signature w-6 text-center text-sm"></i>
                        <span class="nav-text ml-3 text-sm font-bold tracking-wide text-nowrap">Instruksi Surat</span>
                    </a>

                    <div class="menu-header px-4 py-3 text-[10px] font-black text-emerald-200/50 uppercase tracking-[0.2em] mt-4">Monitoring</div>
                    <a href="{{ route('pimpinan.monitoring-riwayat.index') }}" class="nav-item flex items-center py-4 px-5 rounded-2xl transition-all {{ Request::is('pimpinan/monitoring_riwayat*') ? 'sidebar-active' : 'hover:bg-white/10' }}">
                        <i class="fas fa-shoe-prints w-6 text-center text-sm"></i>
                        <span class="nav-text ml-3 text-sm font-bold tracking-wide text-nowrap">Tracking Surat</span>
                    </a>
                    <a href="{{ route('pimpinan.monitoring-arsip.index') }}" class="nav-item flex items-center py-4 px-5 rounded-2xl transition-all {{ Request::is('pimpinan/monitoring_arsip*') ? 'sidebar-active' : 'hover:bg-white/10' }}">
                        <i class="fas fa-file-shield w-6 text-center text-sm"></i>
                        <span class="nav-text ml-3 text-sm font-bold tracking-wide text-nowrap">Monitoring Arsip</span>
                    </a>
                @endif

                <!-- LAPORAN (Tersedia untuk semua yang butuh statistik) -->
                <div class="menu-header px-4 py-3 text-[10px] font-black text-emerald-200/50 uppercase tracking-[0.2em] mt-4">Laporan</div>
                <a href="{{ route(auth()->user()->role . '.statistik') }}" class="nav-item flex items-center py-4 px-5 rounded-2xl transition-all {{ Request::is('*/statistik') ? 'sidebar-active' : 'hover:bg-white/10' }}">
                    <i class="fas fa-chart-pie w-6 text-center text-sm"></i>
                    <span class="nav-text ml-3 text-sm font-bold tracking-wide text-nowrap">Statistik & Laporan</span>
                </a>
            </nav>
        ...
        <main class="flex-1 flex flex-col min-w-0 h-screen overflow-hidden">
            <!-- Header -->
            <header class="h-20 bg-white dark:bg-emerald-900 border-b border-emerald-50 dark:border-emerald-800 shadow-sm flex justify-between items-center px-8 z-20 shrink-0 transition-colors">
                <div class="flex items-center gap-4">
                    ...
                    <div class="flex flex-col leading-none">
                        ...
                        <p class="text-[9px] font-bold text-slate-400 dark:text-emerald-400/60 uppercase tracking-[0.25em] mt-1.5 hidden sm:block">Aksara • Tata Kelola Arsip</p>
                    </div>
                </div>
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div class="flex items-center gap-4 bg-slate-50 dark:bg-emerald-800/50 py-1.5 pl-4 pr-1.5 rounded-2xl border border-slate-100 dark:border-emerald-700">
                        ...
                        <img src="https://ui-avatars.com/api/?name={{ Auth::user()->username }}&background=008f5d&color=fff&bold=true" class="w-10 h-10 rounded-xl shadow-sm border-2 border-white dark:border-emerald-700">
                    </div>
                </div>
            </header>

            <!-- Page Content -->
            <div class="flex-1 overflow-y-auto p-8 custom-scrollbar bg-[#f8fafc] dark:bg-emerald-950/50">
