...
<div class="p-4 md:p-6 space-y-6 animate__animated animate__fadeIn">
    ...
    <div class="bg-white dark:bg-emerald-900/40 p-6 md:p-8 rounded-[2rem] shadow-xl border border-emerald-50 dark:border-emerald-800/50">
        <div class="flex justify-between items-center mb-8">
            ...
            <a href="{{ route('admin.master.instruksi.create') }}" class="bg-emerald-900 dark:bg-emerald-600 text-emerald-50 px-6 py-3 rounded-xl font-black uppercase text-xs hover:bg-emerald-800 dark:hover:bg-emerald-500 transition-all flex items-center gap-2 shadow-lg">
            ...
            <table class="w-full text-left border-separate border-spacing-y-4">
                ...
                <tbody>
                    ...
                    <tr class="bg-emerald-50/50 dark:bg-emerald-950/30 hover:bg-emerald-100 dark:hover:bg-emerald-900/50 transition-all duration-300 rounded-2xl group shadow-sm">
                        <td class="px-6 py-4 font-bold text-emerald-900 dark:text-emerald-100 rounded-l-2xl group-hover:text-emerald-700 dark:group-hover:text-emerald-300">{{ $item->nama_instruksi }}</td>
                        ...
                        <td class="px-6 py-4 text-center rounded-r-2xl">
                            <div class="flex justify-center items-center gap-3">
                                <a href="{{ route('admin.master.instruksi.edit', $item->id_instruksi) }}" 
                                ...
                                <form id="delete-form-{{ $item->id_instruksi }}" action="{{ route('admin.master.instruksi.destroy', $item->id_instruksi) }}" method="POST">
                                    @csrf 
                                    @method('DELETE')
                                    <button type="button" onclick="confirmDelete('{{ $item->id_instruksi }}')"
                    ...
                    @empty
                    <tr>
                        <td colspan="3" class="text-center py-10 text-emerald-400 font-bold italic">Belum ada data instruksi yang tersedia.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
...
<script>
    ...
    function confirmDelete(id) {
        Swal.fire({
            ...
            text: "Data instruksi ini akan dihapus permanen.",
