...
@section('title', 'Tambah Instruksi Pimpinan - AKSARA')
...
<div class="p-4 md:p-6 space-y-6 animate__animated animate__fadeIn max-w-3xl mx-auto">
    ...
    <div class="bg-gradient-to-r from-emerald-900 to-emerald-700 dark:from-emerald-950 dark:to-emerald-900 rounded-[2rem] p-8 text-white shadow-2xl relative overflow-hidden transition-all duration-300">
        <div class="relative z-10">
            <h1 class="text-3xl font-black uppercase tracking-tight italic text-white">TAMBAH INSTRUKSI</h1>
    ...
    <div class="bg-white dark:bg-emerald-900/40 p-6 md:p-8 rounded-[2rem] shadow-xl border border-emerald-50 dark:border-emerald-800/50 transition-all duration-300">
        <form action="{{ route('admin.master.instruksi.store') }}" method="POST" class="space-y-6">
            @csrf
            
            <div class="grid grid-cols-1 gap-6">
                {{-- Nama Instruksi (Dulu Judul) --}}
                <div>
                    ...
                    <input type="text" name="nama_instruksi" value="{{ old('nama_instruksi') }}" required placeholder="Contoh: Percepatan Proses Tender" 
                        class="w-full bg-emerald-50 dark:bg-emerald-950 border-2 border-transparent focus:border-emerald-500 dark:border-emerald-800 rounded-xl px-4 py-3.5 text-sm font-semibold text-emerald-900 dark:text-white outline-none transition-all">
                </div>
                ...
                <div>
                    ...
                    <textarea name="deskripsi" rows="4" placeholder="Detail instruksi pimpinan..." 
                        class="w-full bg-emerald-50 dark:bg-emerald-950 border-2 border-transparent focus:border-emerald-500 dark:border-emerald-800 rounded-xl px-4 py-3.5 text-sm font-semibold text-emerald-900 dark:text-white outline-none transition-all">{{ old('deskripsi') }}</textarea>
                </div>
            </div>
